...
int main()
{
  ...
  for(int i=0; i<NUM; i++){
    ...
    fgets(Strings[i], LEN, stdin); //len sets max read, stdin input
  }
  ...
  */
  ...
  for (int i = 0; i < Strings; i++) {
     for(int j = i + 1; i < Strings; j++) {
        int comparator = strcmp(Strings[i], Strings[j]);
        if (comparator > 0) {
           char temp = Strings[i];
           Strings[i] = Strings[j];
           Strings[j] = temp;
  ...
  /* Write code here to bubble sort the strings in ascending
   alphabetical order
